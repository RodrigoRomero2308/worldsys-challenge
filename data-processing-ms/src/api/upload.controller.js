...
const uploadFile = (req, res) => {
  ...
  bb.on('file', (name, fileStream, info) => {
    ...
    processingStatus[requestId].status = 'uploading_to_disk';

    const tempFilePath = path.join(UPLOADS_DIR, `${requestId}-${filename}`);
    ...
    writeStream.on('finish', () => {
      logger.info(
        `[Controller][${requestId}] Archivo ${filename} guardado en ${tempFilePath}.`
      );
      processingStatus[requestId].status = 'processing_queued';
      ...
      const worker = new Worker(workerPath, {
        workerData: { requestId, filename, tempFilePath },
      });
      ...
      worker.on('message', (message) => {
        ...
        if (message.status === 'completed') {
          logger.info(
            `[Controller][${requestId}] Worker ha completado el procesamiento. Resumen:`,
            message.summary
          );
          ...
          fileStream.on('data', (chunk) => {
            logger.debug(
              `[Controller][${requestId}] Chunk recibido. Pausando stream y enviando a worker.`
            );
            worker.postMessage({
              type: 'PROCESS_CHUNK',
          ...
          logger.debug(
            `[Controller][${requestId}] ACK de chunk recibido. Reanudando stream.`
          );
        }
      });
      ...
      worker.on('exit', (code) => {
        if (code !== 0) {
          ...
          if (
            ...
            processingStatus[requestId].error =
          ...
          logger.info(
            `[Controller][${requestId}] Worker finalizó exitosamente.`
          );
        }
      });
      ...
      writeStream.on('error', (err) => {
        logger.error(
          `[Controller][${requestId}] Error al escribir el archivo en disco:`,
          err
        );
        if (processingStatus[requestId]) {
          processingStatus[requestId].status = 'failed_disk_write';
          processingStatus[requestId].error = err.message;
        }
      });
    });
